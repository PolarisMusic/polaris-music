...
class APIServer {
    ...
    setupRESTEndpoints() {
         ...
         *
         * Checks all critical pipeline services and returns detailed health information.
         *
         * Response format:
         * {
         ...
         *   "timestamp": "ISO8601",
         *   "services": {
         *     "ipfs": [                 // Array of IPFS nodes with details
         *       {
         *         "url": "http://ipfs:5001",
         *         "ok": true,
         *         "id": "QmXxxx...",    // IPFS peer ID
         *         "version": "0.24.0"    // IPFS version
         *       }
         *     ],
         *     "neo4j": { "ok": true },
         *     "redis": { "ok": true },
         ...
         *
         * The event is stored without `sig` and linked to a blockchain transaction
         * via the subsequent POST /api/events/confirm-anchor call.
         *
         * Request body: Event object (without sig) + optional expected_hash
         * Response: Same as /api/events/create
         */
        this.app.post('/api/events/store-for-anchor', this.writeRateLimiter, async (req, res) => {
            ...
            try {
                ...
                // Store event without sig (anchor-auth mode)
                const result = await this.store.storeEvent(event, expected_hash || null, { anchorAuth: true });
         ...
         *
         * This endpoint links the stored event to its blockchain transaction,
         * completing the anchor-auth flow. The on-chain transaction serves as
         * the authoritative proof of authorship.
         *
         * Request body:
         * {
         *   hash: string,            // Event hash
         *   event_cid: string,       // IPFS CID of event
         *   trx_id: string,          // Blockchain transaction ID
         *   author_account: string,  // Blockchain account that submitted the tx
         *   author_permission: string // Permission used (e.g. "active")
         * }
         */
        this.app.post('/api/events/confirm-anchor', this.writeRateLimiter, async (req, res) => {
            ...
            try {
                ...
                if (this.store.redis) {
                    try {
                        ...
                        await this.store.redis.hmset(anchorKey, {
                            trx_id,
                            author_account,
                            author_permission: author_permission || 'active',
                            event_cid: event_cid || '',
                        ...
                        await this.store.redis.expire(anchorKey, 30 * 24 * 60 * 60); // 30 days TTL
                    } catch (redisErr) {
        ...
        // ========== CHAIN READER ENDPOINTS ==========

        // Register chain reader routes (likes, vote tallies)
        // These proxy blockchain table reads through the backend so the browser
        // never needs direct RPC access to chain nodes (eliminates CSP issues)
